<template>
  <div class="ic-container push-from-header">
    <div class="ic-container">
      <div class="grid grid-cols-1">
        <div class="">
          <h1 class="text-6xl font-bold font-display mb-5">
            Nous créons des
            <span class="text-secondary">Collection thématique </span>
            percutantes qui se démarquent...
          </h1>
          <p class="text-2xl font-light font-display">
            Dans le cadre du projet Imagine nous nous proposons de réfléchir à
            la meilleure manière d’exploiter l’archive du collectif Cinéma
            Mémoire. Cette archive, le produit de seize années de travail, est
            un fond riche en termes de contenu et de contenant.
          </p>
        </div>
      </div>
    </div>
    <div class="ic-container mt-24">
      <h1 class="sm:text-3xl md:text-5xl font-bold font-display">
        Nos collections thématiques
      </h1>
    </div>
    <div class="ic-container flex flex-col gap-y-64 mt-24">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-16 card-item">
        <div data-scroll data-scroll-speed="1">
          <img
            class="img-wrapper"
            src="https://images.unsplash.com/photo-1499720843949-d9e6f318dca0?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60"
            alt="title"
          />
        </div>
        <div class="marge">
          <div data-scroll data-scroll-speed="3">
            <h2 class="card-title p-4 bg-white">This is just a title</h2>
          </div>
          <div class="pt-24" data-scroll data-scroll-speed="4">
            <p class="discreption" id="desc">
              Lorem ipsum dolor, sit amet consectetur adipisicing elit.
              Excepturi inventore vitae hic earum provident.
            </p>
            <span> En savoir plus </span>
          </div>
        </div>
      </div>
      <!-- <div class="grid grid-cols-1 md:grid-cols-2 gap-16">
        <div class="title-cn marge">
          <div class="text-right pb-10" data-scroll data-scroll-speed="3">
            <h2 class="card-title-rev sm:text-left md:text-right p-4 bg-white">
              This is just a title
            </h2>
          </div>
          <p class="pt-24 text-right" data-scroll data-scroll-speed="4">
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Excepturi
            inventore vitae hic earum provident. Quae adipisci repellat
            assumenda minus debitis, iusto veritatis! Suscipit blanditiis,
            facilis ea vel animi exercitationem itaque.
          </p>
        </div>
        <div class="" data-scroll data-scroll-speed="1">
          <img
            src="https://images.unsplash.com/photo-1506259091721-347e791bab0f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60"
            alt="title"
          />
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-16">
        <div class="" data-scroll data-scroll-speed="1">
          <img
            src="https://images.unsplash.com/photo-1550684848-fac1c5b4e853?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60"
            alt="title"
          />
        </div>
        <div class="marge">
          <div data-scroll data-scroll-speed="4">
            <h2 class="card-title p-4 bg-white">
              An Encouter with Colors to be seen
            </h2>
          </div>

          <p
            class="pt-24"
            data-scroll
            data-scroll-speed="3"
            data-scroll-offset-bottom="25%, 15%"
          >
            Lorem ipsum dolor, sit amet consectetur adipisicing elit. Excepturi
            inventore vitae hic earum provident. Quae adipisci repellat
            assumenda minus debitis, iusto veritatis! Suscipit blanditiis,
            facilis ea vel animi exercitationem itaque.
          </p>
        </div>
      </div> -->
    </div>
    <div class="ic-container">
      <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-24 mt-24">
        <div class="">
          <h1 class="sm:text-3xl md:text-3xl font-bold font-display mb-8">
            Nos objectifs a long termes
          </h1>
          <p class="text-base font-regular font-display leading-7">
            L’objectif à long terme, consiste à mettre à disposition de
            différents publics ces travaux, susceptibles de les aider dans leur
            réflexion. D’autres collectifs et organismes vont également
            exploiter cette matière et suivre ce modèle. Cela pourrait être des
            festivals qui proposent des ateliers pendant l’événement ou hors les
            murs, des centres de recherche coutumiers d’effectuer des travaux
            sur les mêmes thématiques.
            <br />
            <br />
            Cette matière peut également permettre la création de passerelles et
            donc des synergies avec certains de ces collectifs, afin de mener
            collectivement des réflexions autour de l’image, son impact et
            comment la créer. Cette réflexion Projet Imagine 5 commune, pourrait
            aboutir à d’autres objets exploitables par les mêmes acteurs ou
            d’autres, de par le monde.
            <br />
            <br />
            La collection thématique pourra servir de support pédagogique lors
            des futurs ateliers de création de films documentaires ou
            d’éducation à l’image organisés par Cinéma Mémoire ou d’autres
            collectifs Ce travail innovant et créatif sera modélisé sous forme
            de guide pour capitaliser les méthodes mises en place et refléter le
            processus qui a été mis en place pour permettre cette production.
          </p>
        </div>
        <div class="">
          <h1 class="sm:text-3xl md:text-3xl font-bold font-display mb-8">
            Nos objectifs a court terme
          </h1>
          <p class="text-base font-regular font-display leading-7">
            L’objectif à court terme, consiste à créer une collection thématique
            sous forme d’un objet image et texte. Cet objet sera mis en ligne
            sur la plate-forme numérique IMAGINE de Cinéma &amp; Mémoire.
            <br />
            <br />
            Il s’agit d’exposer et de dévoiler le raisonnement de ce collectif
            autour de la place de l’image dans la société et le rôle qu’elle
            peut jouer dans sa transformation et son évolution. Il s’agit
            également de montrer le processus de création, pas uniquement sur le
            plan technique, mais également sur le plan artistique.
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import gsap from 'gsap'
import SmoothScroll from '~/mixins/SmoothScroll.js'

export default {
  mixins: [SmoothScroll],
  mounted() {
    const card = document.querySelector('.card-item')
    const img = document.querySelector('.img-wrapper')
    const desc = document.querySelector('.discreption')

    const tl = gsap.to(img, {
      scale: 0.98,
      duration: 0.5,
      paused: true,
      ease: 'power1.inOut'
    })
    const hover = gsap.to(desc, {
      scale: 1.5,
      color: 'blue',
      duration: 0.5,
      paused: true,
      ease: 'power1.inOut'
    })

    card.addEventListener('mouseenter', () => {
      tl.play()
      hover.play()
    })
    card.addEventListener('mouseleave', () => {
      tl.reverse()
      hover.reverse()
    })
  }
}
</script>

<style lang="postcss" scoped>
.discreption {
  overflow: hidden;
  color: black;
}
.head-title-block {
  @apply w-full flex flex-col mx-auto;
}

@screen lg {
  .head-title-block {
    @apply w-8/12;
  }
}

.card-title {
  position: absolute;
  @apply text-4xl font-extrabold leading-none mt-4;
  transform: translateX(-10vw);
  color: black;
  text-shadow: 0px 4px 10px rgba(238, 238, 238, 0.25);
}

.card-title-rev {
  position: absolute;
  @apply text-4xl font-extrabold leading-none mt-4;
  transform: translateX(200px);
  z-index: 10;
  text-shadow: 0px 4px 10px rgba(238, 238, 238, 0.25);
}
.title-cn {
  z-index: 100;
}

.marge {
  margin-top: 6rem;
}

@media only screen and (max-width: 600px) {
  .card-title {
    position: absolute;
    @apply text-4xl font-extrabold leading-none mt-4;
    transform: translateX(0);
    color: black;
  }
  .card-title-rev {
    position: absolute;
    @apply text-5xl font-extrabold leading-none mt-4;
    transform: translateX(0);
    text-align: left;
    z-index: 10;
  }
  .marge {
    margin-top: 0;
  }
}
</style>
